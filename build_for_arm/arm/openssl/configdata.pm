...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/home/kevin/workspace/1.code/gateway/code/Dalitek-IOT-GW/build/arm/usr no-asan no-asm no-crypto-mdebug no-crypto-mdebug-backtrace no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-tls1_3 no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl => "/usr/bin/perl",
  perlargv => [ "no-asm", "--prefix=/home/kevin/workspace/1.code/gateway/code/Dalitek-IOT-GW/build/arm/usr", "linux-generic32", "no-shared" ],
  prefix => "/home/kevin/workspace/1.code/gateway/code/Dalitek-IOT-GW/build/arm/usr",
  processor => "",
